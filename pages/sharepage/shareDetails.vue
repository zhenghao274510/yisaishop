<template>
	<view class="">
		<view class="details_page" v-if="isShow==1">
			...
			<view class="goods_info_use" style="margin-bottom:14upx;" v-if="type==1 && dataobject.getGroupOrderList.length!=0">
				...
				<view class="" style="padding-top:10px;">
					<view class="uni-flex d-items-center d-between" style="padding: 20upx 0;" v-for="(v,k) in dataobject.getGroupOrderList"
					 :key="k">
						<view class="uni-flex d-items-center" v-if="k==0">
						...
						<view class="uni-flex d-items-center">
							<view class="" style="margin-right: 50upx;">
								...
								<view class="uni-flex d-items-center">
									<bbs-countdown :time="dataobject.end_time" type='1'></bbs-countdown>
								</view>
							</view>
			...
			<view class="goods_info_use" style="margin-bottom:14upx;background: #FFFFFF;">
				<view class="uni-flex d-items-center d-between">
					...
					<navigator :url="`../mineoptions/productEavList?id=${id}`" class="uni-flex d-items-center">
						<view style="font-size:24upx ;color:#999999;">查看更多</view>
			...
			<group-goods-nav v-if="type==1" @buttonClick='buttonClick' @click="itemClick" :dadaobject='dataobject'></group-goods-nav>
			...
			<!-- #endif -->
			<more-group-sku :dataobject="dataobject" ref="moreGroupSku" @click="choseGroupNumber"></more-group-sku>
			...
			<!-- 底部操作 -->
			<hch-poster ref='hchPoster' :dataobject="dataobject"></hch-poster>
		</view>
		<no-shop :isShow="isShow"></no-shop>
	</view>
	...
	export default {
		...
		onLoad(options) {
			...
			if (options.q) {
				...
				const parmas = decodeURIComponent(options.q);
		...
		methods: {
			getUrlCode(parmas) { // 截取url中的code方法
			...
			itemClick(e) {
				...
				//  #ifdef H5
				e.index == 0 ? (getApp().globalData.tabIndex = 2,
					this.$api.reLaunch('/pages/index/index')) : this.$refs['hchPoster'].open()
				// #endif
		...
		onShareAppMessage() {
			...
			return {
				title: this.dataobject.title,
				path: `/pages/details/goodsDetails?id=${this.id}&type=${this.type}`
			}
		}
		//#endif
	}
